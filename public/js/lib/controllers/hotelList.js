...
hotelList.directive('hotelStar', function($compile){
	return function (scope, element, attrs){
		...
		if((attrs.rateex.indexOf("STAR")!=-1)|| (attrs.rateex.indexOf("*")!=-1)){
			...
			var x = $rate.split(" ");  $rateSup= x[x.length - 1];
			if(isNaN($stars)){
				$stars=parseInt($rateSup.substring(0,1));
				$rateExe=$rate.replace($rateSup,'');
			}
			//var n = $rateEx.split(" "); $rateE= n[n.length - 1];
			if(($rateSup.indexOf("STAR")==-1)&&($rateSup.indexOf("*")==-1)){  $rateExe=$rateSup};
		}
		else{$stars=0; $rateExe=attrs.rateex}
		$ratehtml='';
		for($r=0;$r<$stars;$r++){$ratehtml=$ratehtml+' <span class="icon-star"></span>';}
		$ratehtml= $ratehtml +'<span> '+$rateExe+'</span>'
		element.context.innerHTML=$ratehtml;
	}
})
hotelList.controller('hotelList', ['$scope', '$route', 'searchDatas', 'hotelData',  'purchaseData', 'travelPackD',  'hotel_room_allo','hotelShortDetailsRs', 'currencyData', '$location', 'hotelFprox', 'hotelChain', 'hotelContact', '$http', function($scope, $route, searchDatas, hotelData, purchaseData, travelPackD,  hotel_room_allo, hotelShortDetailsRs, currencyData, $location, hotelFprox, hotelChain, hotelContact, $http) {
  ...
  $scope.sort='lowP';
  $scope.pInt='All';  $scope.pChain='All';
  $scope.hClist=[];
  $scope.hotels=[];
  $scope.hotelList=[];
  ...
  $scope.sorting=[{name:'Lowest Price', value:'lowP'},{name:'Highest Price', value:'highP'},{name:'Lowest Star Rating', value:"hotelCat['@shortname']"},{name:'Higest Star Rating', value:"-hotelCat['@shortname']"},{name:'Hotel Name Asc.', value:'hotelName'},{name:'Hotel Name Desc.', value:'-hotelName'}];
	$scope.priceMin = function (hotel) {
       if(hotel.availRoom instanceof Array){  return parseFloat(hotel.availRoom[0].HotelRoom.Price.Amount) >= $scope.minPrice;}
	   else{return parseFloat(hotel.availRoom.HotelRoom.Price.Amount) >= $scope.minPrice;}
    };
	$scope.priceMax = function (hotel) {
		if(hotel.availRoom instanceof Array){ return parseFloat(hotel.availRoom[0].HotelRoom.Price.Amount) <= $scope.maxPrice;}
		else{return parseFloat(hotel.availRoom.HotelRoom.Price.Amount) <= $scope.maxPrice;}
    };
	$scope.pOfInt = function (hotel) {
		if( $scope.pInt=='All'){ return true}
		else{ for($i=0; $i<hotel.filter[0].length; $i++){
				if(hotel.filter[0][$i]==$scope.pInt){return true}
			}
		}
    };
	$scope.pChainName = function (hotel) {
		if( $scope.pChain=='All'){ return true}
		else{
            for($i=0; $i<hotel.filter[1].length; $i++){
			    if(hotel.filter[1][$i]==$scope.pChain){return true}
			}
		}
    };
	$scope.hotelName = function (hotel) {
		if( $scope.hotel_name==''){ return true}
		else{  return (hotel.HotelInfo.Name.toLowerCase().search($scope.hotel_name.toLowerCase())>-1)
				//else{return false}
		}
    };
	...
	$scope.sortHotel=function(hotel){
		if($scope.sort=='lowP'){
			if(hotel.availRoom instanceof Array){  return parseFloat(hotel.availRoom[0].HotelRoom.Price.Amount);}
		    else{return parseFloat(hotel.availRoom.HotelRoom.Price.Amount)}
		}
		else if($scope.sort=='highP'){
			if(hotel.availRoom instanceof Array){ return -parseFloat(hotel.availRoom[0].HotelRoom.Price.Amount)}
		    else{return -parseFloat(hotel.availRoom.HotelRoom.Price.Amount)}
		}
		else if($scope.sort=='lowR'){return hotel.HotelInfo.Category["@shortname"];}
		else if($scope.sort=='highR'){return ('-'+hotel.HotelInfo.Category["@shortname"]);}
		else if($scope.sort=='lowN'){return hotel.HotelInfo.Name;}
		else if($scope.sort=='highN'){return ('-'+hotel.HotelInfo.Name);}
	}
	...
	for (property in $scope.hotels){//loop through the room_avail to get all room occupancies for this room
	  if($scope.hotels.hasOwnProperty(property)){
		  ...
		  else{var properti=$scope.hotels;}
  ...
  $scope.getProximity=function(){
		$scope.hprox= hotelFprox.query({hotelCodes:JSON.stringify($scope.hClist)}, function(hprox){
			for (property in hprox){//loop through the room_avail to get all room occupancies for this room
			  if(hprox.hasOwnProperty(property)){
				  if(hprox instanceof Array){var properti=hprox[property];}
				  else{var properti=hprox;}
    ...
    $scope.getChain=function(){
		$scope.hchain= hotelChain.query({hotelCodes:JSON.stringify($scope.hClist)}, function(hchain){
			for (property in hchain){//loop through the room_avail to get all room occupancies for this room
			  if(hchain.hasOwnProperty(property)){
				  if(hchain instanceof Array){var properti=hchain[property];}
				  else{var properti=hchain;}
  ...
  $scope.getHdetails=function(e, a, b){
	   $scope.hsdet= hotelShortDetailsRs.get({hotel_code:a, cat_code:b}, function(hsdet){
			...
			if (e.parent().find('.h_rate')[0].innerHTML ==""){e.parent().find('.h_rate')[0].innerHTML=hsdet.cat}
		})
  }
  ...
  $scope.sethotels_details=function(code_token, index_num){
	...
	$scope.book_room=function(code_token, index_num, room_index){
		...
		if($hot.tag=='HotelBed'){
			...
			hdata= { hotel_code:$hot.hotelCode, check_in :$hot.dateFrom, check_out:$hot.dateTo, sequence_num:$hot.S_num, selected_room:$selected_rooms
				}
		}
		$http({method:'Post', url:$bookurl, data:hdata}).then(function successCallback(response) {
				...
				if($hot.tag=='HotelBed'){
					...
					hotelData.setData($scope.serv);
                        ...
                        $scope.services={};
                        $hot.availRoom[0].HotelRoom.CancellationPolicies=response.data;
						$scope.services.availRoom=$hot.availRoom[0];
						$scope.services.TotalAmount=$hot.availRoom[0].HotelRoom.Price.Amount;
				...
				for($a=0; $a<$scope.serv.length; $a++){
					if($scope.serv[$a].HotelInfo.Code==$hot.hotelCode){
						...
						travel_pack={ product:$hot.tag, productType:'Hotel', Adult:$scope.search_c.hAdult, Child:$scope.search_c.hChild,
				                        hdesdesc:$scope.search_c.hdesdesc,	hcheckin:$hot.dateFrom, hcheckout:$hot.dateTo,
                                        hcheckinL:$scope.search_c.moduleCurrType[1].value.long, hcheckoutL:$scope.search_c.moduleCurrType[2].value.long,
                                        imgurl:$hot.hotelImages[0], Name:$hot.hotelName,  hRoom:$scope.search_c.moduleCurrType[3].value,
                                        Price:$scope.serv[$a].TotalAmount,  guestBreak:$scope.roomz,  total_nights:$scope.search_c.moduleCurrType[2].value.fTravelDays,
                                        hroomdist:$scope.search_c.moduleCurrType['occupancy'],  ref:null, serviceRef:null, supplier:null,
  ...
  return {
	...
	controller: function($scope, $attrs, hotel_room_allo) {
		...
		function roomboard(room_all){
		  ...
		  for($i=0; $i<$scope.search_c.moduleCurrType[3].value; $i++){ // this loop create different array according to Occupancy in each room
			 var ind=0;
