...
TravelPack.controller("TravelPack", [
	"$scope",
	"$rootScope",
	"searchDatas",
	"travelPackD",
	...
	"serviceRemoveRs",
	"currencyData",
	"currencyConvertFilter",
	"$modal",
	function($scope, $rootScope, searchDatas, travelPackD, flightCheckRs, serviceRemoveRs, currencyData, currencyConvertFilter, $modal) {
		...
		$scope.stripTravelPD = function(data) {
			...
			for ($i = 0; $i < data.length; $i++) {
				...
				if (data[$i].product == "HotelBed" || (data[$i].product == "Juniper" && data[$i].status != "booked")) {
					...
					data[$i].idc = $i;
					if (data[$i].productType == "Hotel") {
						$scope.hotelD.push(data[$i]);
					} else if (data[$i].productType == "Tour") {
						$scope.tourD.push(data[$i]);
					} else {
						$scope.transferD.push(data[$i]);
					}
		...
		$scope.serRemove = function(spui, ptoken, index) {
			...
			$scope.sRemove = serviceRemoveRs.get({ spui: spui, pToken: ptoken }, function(sRemove) {
				alert("Removed");
...
TravelPack.controller("TPfList", [
	"$scope",
	"searchData",
	"flightListRs",
	function($scope, searchData, flightListRs) {
		$scope.getData = searchData.data();
		$scope.offer = "";
		$scope.fList = flightListRs.get(
			{
				Adult: $scope.getData[0].Adult,
				Child: $scope.getData[0].Child,
				fSeniors: $scope.getData[0].fSenior,
				Infant: $scope.getData[0].Infant,
				fDepCode: $scope.getData[0].fDepCode,
				fDesCode: $scope.getData[0].fDesCode,
				fTravelDays: $scope.getData[0].fTravelDays,
				fDepDate: $scope.getData[0].fDepDate
			},
			function(fList) {
...
TravelPack.controller("TPhList", [
	"$scope",
	"searchData",
	"hotelListRs",
	function($scope, searchData, hotelListRs) {
		$scope.getData = searchData.data();

		$scope.offer = "";
		$scope.hList = hotelListRs.get(
			{
				hAdult: $scope.getData[17].valueData,
				hChild: $scope.getData[18].valueData,
				hDesCode: $scope.getData[20].valueData,
				hcheckin: $scope.getData[22].valueData,
				hcheckout: $scope.getData[23].valueData,
				hRoomBreakDown: JSON.stringify($scope.getData[27])
			},
			function(hList) {
...
TravelPack.controller("TPtList", [
	"$scope",
	"searchData",
	"tourData",
	"tourListRs",
	function($scope, searchData, tourData, tourListRs) {
		$scope.getData = searchData.data();
		...
		function search_tour() {
			var tour = {};
			$scope.tList = tourListRs.get(
				{ hDesCode: $scope.getData[20].valueData, hTourin: $scope.getData[22].valueData, hTourout: $scope.getData[23].valueData, tourBreakDown: JSON.stringify($scope.getData[28]) },
				function(tList) {
...
TravelPack.controller("conditionModalInstanceCtrl", [
	"$scope",
	"$modalInstance",
	"offer",
	"search_c",
	"flightCondRs",
	function($scope, $modalInstance, offer, search_c, flightCondRs) {
...
TravelPack.controller("TPtrList", [
	"$scope",
	"searchData",
	"transferData",
	"transferListRs",
	"transferServiceAddRs",
	function($scope, searchData, transferData, transferListRs, transferServiceAddRs) {
		$scope.getData = searchData.data();
		...
		if ($scope.children > 0) {
			while ($scope.guest[1].push([]) < $scope.children);
		}
		search_transfer();
		...
		function search_transfer() {
			...
			$scope.tList = transferListRs.get(
				{
					hDesCode: $scope.getData[5].valueData,
					hDesType: $scope.getData[6].valueCountry,
					hReturnCode: $scope.getData[8].valueData,
					hReturnType: $scope.getData[9].valueCountry,
					hReturnOption: $scope.getData[9].valueName,
					...
					transferBreakDown: JSON.stringify($scope.getData[28])
				},
				function(tList) {
					...
					if ($scope.transfers_total != 0) {
						$scope.getData[26].availToken = $scope.transfers[0]["@availToken"];
					}
				}
			);
		}
